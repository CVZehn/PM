...
{ 
     ...
     OLED_SDIN_Clr();//OLED_SDA = low;OLED_SDA = low;
	 delay_us(1);
    OLED_SCLK_Clr();//OLED_SCL = low; OLED_SCL = low;
   delay_us(1);
    OLED_SDIN_Set();//OLED_SDA = high;OLED_SDA = high;
	 delay_us(1);
   OLED_SCLK_Set();//OLED_SCL = high;OLED_SCL = high;
}
...
void Write_IIC_Byte(unsigned char IIC_Byte)
{
  ...
  for(i=0;i<8;i++)
	...
	OLED_SCLK_Set();//OLED_SCL = high;OLED_SCL=1;
    delay_us(1);
	 OLED_SCLK_Clr();//OLED_SCL = low;OLED_SCL=0;
}
